...
//  BLADS
//
//  Created by Keith Birthday on 12/1/13.
//  Copyright 2013 Keith Birthday. All rights reserved.
//
...
-(id) init
{
	if( (self=[super init])) {
        ...
        //add background
        CCSprite* background = [CCSprite spriteWithFile:@"technodrome.jpg"];
        background.position = ccp(size.width/2, size.height/2);
        [background setScaleX:size.width/background.contentSize.width];
        [background setScaleY:size.width/background.contentSize.width];
        [self addChild:background z:0];
        
        //Scoring
        score = 0;
        scoreLabel = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%i", score]
                        fontName:@"Helvetica Neue" fontSize:32];
        [scoreLabel setColor:ccc3(255,255,255)];
        scoreLabel.position = ccp(size.width/9, size.height/2);
        ...
        playerSprite = [CCPhysicsSprite spriteWithFile:@"bill.png"];
        float playerScale = 1;
        ...
        //Cause blocks to fall every interval
...
//	[parent addChild:sprite];
    ...
    CCPhysicsSprite *sprite = [CCPhysicsSprite spriteWithTexture:spriteTexture_ rect:CGRectMake(0,0,20,31)];
	[parent addChild:sprite];
    sprite.tag = obstacle;
